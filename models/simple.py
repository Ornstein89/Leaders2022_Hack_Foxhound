...
class SimpleModel:
    ...
    def _load_dicom(self, file_path):
        ...
        dicom_names = reader.GetGDCMSeriesFileNames("../input/dicom-test/2.000000-5mm-81154")
        reader.SetFileNames(dicom_names)

        image = reader.Execute()
        return image

    def _synthetic_image(self, cts_infected, lungs_infected, infection_mask, cts_normal, lungs_normal, layers):
        """
    ...
    def _recover_synthetic_image(
            ...
            synthetic_mask, layers, normal_lung
        ):
        images = []
        masks = []
        j = 0
        for i in np.arange(image.shape[0]):
            if i in layers:
                images.append(
                    self._lung_with_bg(image[i], synthetic[j], normal_lung[i])[np.newaxis])
