...
class Pix2Pix:
    """Модель использующая pix2pix апхитекрутуру для генерации по скетчу с лёгким изображения.
    ...
    Methods:
        inference : открывает dicom файлы (N, H, W) и генерирует по нему, возращаея КТ-скан (N, H, W) и маску (N, H, W)
        ... 
    """
    ...
    def inference(self, dicom_path, original_mask=None):
        """Открывает dicom файлы (N, H, W) и генерирует по нему, возращаея 
        сгенерированный КТ-скан (N, H, W) и маску (N, H, W).
        
        Args:
            dicom_path : str : путь к папке с dicom файлами.
            original_mask : ndarray : размер (K, 512, 512), куб с масками по которые будут братся для модели.
